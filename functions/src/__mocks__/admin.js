...
const mFirestore = {
    ...
    set: jest.fn(async (d, transactionD) => {
        let { queries, path, data } = mFirestore;
        if (isTransaction) d = transactionD;
        queries.push(path);
    ...
    update: jest.fn(async (d, transactionD) => {
        let { queries, path, data } = mFirestore;
        if (isTransaction) d = transactionD;
        queries.push(path);
 ...
 isTransaction = true; return await fn(mFirestore);
}),
    ...
    reset: () => {
 mFirestore.data = {}; mFirestore.path = []; mFirestore.queries = [];
},
};
