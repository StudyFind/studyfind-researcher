...
import { TimeInput } from "@studyfind/components";
...
function ReminderTimesInput({ times, error, handleChange }) {
  ...
  const handleCheckTime = () => {
    if (time === "") return 'Please select a time and click the "Add Time" button';
  ...
  return (
    <>
      <Flex gridGap="10px" align="flex-start">
        <TimeInput name="time" value={time} error={localError} onChange={(n, v) => setTime(v)} />
        <Button variant="outline" colorScheme="blue" px="20px" onClick={handleAddTime}>
          Add Time
        </Button>
      </Flex>
