...
function RemindEdit({
  ...
  const handleSubmit = () => {
    ...
    const thisStartDate = firebase.firestore.Timestamp.fromDate(
      new Date(startYear, startMonth - 1, startDay)
    );
    const [endYear, endMonth, endDay] = endDate.split("-");
    const thisEndDate = firebase.firestore.Timestamp.fromDate(
  ...
  return (
    <>
      ...
      <Row>
        <Text color="tomato">{timeError}</Text>
      </Row>
      <Row_align_left>
        {times.map((time, index) => (
          <Tag key={index}>
      ...
      <Row>
        <Button
          colorScheme={sunday ? "red" : "green"}
          size="md"
          onClick={() => (sunday ? setSunday(false) : setSunday(true))}
        >
          U
        </Button>
        <Button
          colorScheme={monday ? "red" : "green"}
          size="md"
          onClick={() => (monday ? setMonday(false) : setMonday(true))}
        >
          M
        </Button>
        <Button
          colorScheme={tuesday ? "red" : "green"}
          size="md"
          onClick={() => (tuesday ? setTuesday(false) : setTuesday(true))}
        >
          T
        </Button>
        <Button
          colorScheme={wednesday ? "red" : "green"}
          size="md"
          onClick={() => (wednesday ? setWednesday(false) : setWednesday(true))}
        >
          W
        </Button>
        <Button
          colorScheme={thursday ? "red" : "green"}
          size="md"
          onClick={() => (thursday ? setThursday(false) : setThursday(true))}
        >
          R
        </Button>
        <Button
          colorScheme={friday ? "red" : "green"}
          size="md"
          onClick={() => (friday ? setFriday(false) : setFriday(true))}
        >
          F
        </Button>
        <Button
          colorScheme={saturday ? "red" : "green"}
          size="md"
          onClick={() => (saturday ? setSaturday(false) : setSaturday(true))}
        >
          S
        </Button>
      </Row>
      ...
      <Row>
        <Text color="tomato">{startEndTimeError}</Text>
      </Row>
