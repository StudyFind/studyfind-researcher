...
function RemindView({
  ...
  const goToEdit = (reminder, index) => {
    ...
    setStartDate(
      `${reminder.startDate.toDate().getFullYear()}-${
        reminder.startDate.toDate().getMonth() + 1
      }-${reminder.startDate.toDate().getDate()}`
    );
    setEndDate(
      `${reminder.endDate.toDate().getFullYear()}-${
        reminder.endDate.toDate().getMonth() + 1
      }-${reminder.endDate.toDate().getDate()}`
    );
    var repeatance = 0;
...
function startAndEndDate(reminder) {
  ...
  return `${
    months[reminder.startDate.toDate().getMonth()]
  }-${reminder.startDate.toDate().getDate()}-${reminder.startDate.toDate().getFullYear()} to ${
    months[reminder.endDate.toDate().getMonth()]
  }-${reminder.endDate.toDate().getDate()}-${reminder.endDate.toDate().getFullYear()}`;
}
